...
mibVar = rfc1902.ObjectIdentity('BNC-MIB', 'ifInOctets', 1)
...
# a web site.
try:
    mibVar.addAsn1MibSource('BNC.MIB')
...
mibVar = rfc1902.ObjectIdentity(
    (1, 3, 6, 1, 2, 'mib-2', 1, 'sysDescr')).resolveWithMib(mibView)

print(mibVar.prettyPrint(), tuple(mibVar), str(mibVar))

# Obtain MIB object information by a label
mibVar = rfc1902.ObjectIdentity(
    'iso.org.dod.internet.mgmt.mib-2.system.sysDescr').resolveWithMib(mibView)
...
mibVar = rfc1902.ObjectIdentity(
    'SNMPv2-MIB', last=True).resolveWithMib(mibView)
...
print(varBind[0].prettyPrint(),
      varBind[1].__class__.__name__, varBind[1].prettyPrint())
...
print(['%s = %s(%s)' % (x[0].prettyPrint(), x[1].__class__.__name__,
      x[1].prettyPrint()) for x in varBinds])
